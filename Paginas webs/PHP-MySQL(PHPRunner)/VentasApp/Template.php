...
class clsTemplate
{
  ...
  function LoadTemplateFromStr($file_content, $block_name, $encoding = "", $meta = "") {
    ...
    $file_content = preg_replace_callback("/\\{res:\s*(\w+)\\}/is", function($match) use ($CCSLocales, $FileEncoding) { 
      return CCConvertEncoding($CCSLocales->GetText($match[1]), $FileEncoding, $this->internal_encoding);
    }, $file_content);
