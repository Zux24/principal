...
class LoginPage extends RunnerPage 
{
	 ...
	 * @return Boolean
	 */	
	protected function logInHardcoded( $pUsername, $pPassword, $skipCheck = false, $fireEvents = true )
	{
		//	 username and password are hardcoded
		$cUserName = "admin";
		$cPassword = "admin";
		
		if( $skipCheck || $this->checkUsernamePassword( $pUsername, $pPassword ) )
		{
			$_SESSION["UserID"] = $cUserName;
			$_SESSION["UserName"] = runner_htmlspecialchars( $cUserName );
			$_SESSION["AccessLevel"] = ACCESS_LEVEL_USER;
			if( $this->auditObj )
			{
			...
			$dummy = array();
			SetAuthSessionData($pUsername, $dummy, false, $pPassword, $this, $fireEvents );
			return true;
		}
		if( $fireEvents )
	...
	public function LogIn( $pUsername, $pPassword, $skipPasswordCheck = false, $fireEvents = true )
	{
		...
		return $this->logInHardcoded( $pUsername, $pPassword, $skipPasswordCheck, $fireEvents );
	}
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("")
